            ...
            class EGVAR(main,ki_actions)
            {
                ...
                class Attributes
                {
                    class EGVAR(main,sitzen)
                    {
                        ...
                        expression =  QUOTE(if (!is3DEN && _value) then {[{time > 0 && !isNil QQEGVAR(main,initDone)}, {[_this] call FUNC(sitAI)}, _this] call CBA_fnc_waitUntilAndExecute});
                    ...
                    class EGVAR(main,keys)
                    {
                        ...
                        expression =  QUOTE(if (!is3DEN && _value != '[]' && _value != '') then {[{time > 0 && !isNil QQEGVAR(main,initDone)}, {_this call FUNC(keysForAI)}, [_this, _value]] call CBA_fnc_waitUntilAndExecute});
