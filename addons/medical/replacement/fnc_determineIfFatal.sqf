 ...
 * Public: No
 */
// ORIGINAL: https://github.com/acemod/ACE3/blob/master/addons/medical_damage/functions/fnc_determineIfFatal.sqf
...
if (_unit getVariable [QEGVAR(medical,fatalDamageSource), EGVAR(medical,fatalDamageSource)] in [0, 2]) then {
...
if (_unit getVariable [QEGVAR(medical,fatalDamageSource), EGVAR(medical,fatalDamageSource)] in [1, 2]) then {
