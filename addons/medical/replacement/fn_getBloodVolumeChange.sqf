...
if (!isNil {_unit getVariable QGVAR(ivBags)}) then {
    if (_bloodVolume < 100) then {
        ...
        _bloodBags = _bloodBags apply {
            ...
            private _boost = [1, 10] select ((_unit getVariable ["TBMod_main_cprBoost", 0]) > 0);
            ...
            ["RemainIV: %1 | ChangeIV: %2 | InfluCoef: %3 | InFac: %4 | InVeh: %5 | Boost: %6 | bloodVolume: %7", _bagVolumeRemaining, _bagChange, _influence, _inFacility, _inVehicle, _unit getVariable ["TBMod_main_cprBoost", 0], _bloodVolume] call TBMod_main_fnc_debug; // kein Makro für debug da ace Namespace
            _unit setVariable ["TBMod_main_cprBoost", ((_unit getVariable ["TBMod_main_cprBoost", 0]) - 1) max 0];
            // CHANGE ENDE
