...
*/
params [["_input", false, [false]], ["_spawnSelect", true]];
...
[
    ...
    {
        (_this # 1) params ["_uid"];
        GVAR(disconnectCache) getVariable [_uid, []]
    }, {
        params ["_return", "_args"];
        _args params ["", "_spawnSelect"];

        if !(_return isEqualTo []) then
        {
            _return params ["_uid", "_gear", "_pos", "_dir", "_arsenalType", "_rolle", "_group", "_team"];
    ...
    [getPlayerUID player, _spawnSelect]
] call FUNC(transfer);
