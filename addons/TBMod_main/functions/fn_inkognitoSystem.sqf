...
TB_condLaden = {
    params ["_mode"];
    (profileNamespace getVariable ["TB_umziehen", []]) params ["", "", ["_currentMode", ""]];
    !((nearestObjects [ACE_player, ["RDS_S1203_Civ_Base", "RDS_Ikarus_Civ_Base"], 10]) isEqualTo []) &&
...
// Speichern
private _condSpeichern = {!((nearestObjects [ACE_player, ["TB_arsenal_base"], 10]) isEqualTo [])};
