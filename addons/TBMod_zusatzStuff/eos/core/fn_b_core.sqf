...
for "_counter" from 1 to _PApatrols do
{
    ...
    _grp=[_pos,_PAgroupSize,_faction,_side] call TB_EOS_fnc_spawnGroup;    
...
for "_counter" from 1 to _LVehGroups do
{
    ...
    _bGroup=[_newpos,_side,_faction,_vehType]call TB_EOS_fnc_spawnVehicle;                    
    
    if ((_LVgroupSize select 0) > 0) then
    {
        0=[(_bGroup select 0),_LVgroupSize,(_bGroup select 2),_faction,_cargoType] call TB_EOS_fnc_setCargo;
    };

    0=[(_bGroup select 2),"LIGskill"] call TB_EOS_fnc_setSkill;
...
for "_counter" from 1 to _AVehGroups do
{
    ...
    0=[(_cGroup select 2),"ARMskill"] call TB_EOS_fnc_setSkill;    
...
for "_counter" from 1 to _CHGroups do
{
    ...
    if ((_fSize select 0) > 0) then
    {
        ...
        0=[(_fGroup select 0),_fSize,_cargoGrp,_faction,9] call TB_EOS_fnc_setCargo;
        0=[_cargoGrp,"INFskill"] call TB_EOS_fnc_setSkill;
