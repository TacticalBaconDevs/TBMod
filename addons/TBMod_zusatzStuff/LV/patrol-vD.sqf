...
//*Syntax: nul = [this] execVM "\TBMod_zusatzStuff\LV\patrol-vD.sqf";
//     or: nul = [this,center position,radius,handle doors] execVM "\TBMod_zusatzStuff\LV\patrol-vD.sqf";
...
if(isNil("LV_CloseDoors"))then{LV_CloseDoors = compile preprocessFile "\TBMod_zusatzStuff\LV\functions\LV_fnc_closeDoors.sqf";};
...
params [
       ...
       "_cPos",
       ["_radius", 150, [0]],
       ["_handleDoors", false, [false]]
    ];

if (isNil "_cPos") then {_cPos = getPos _unit};
if (isNil "_radius") then {_radius = 150};
