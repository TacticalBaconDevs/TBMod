...
private _state = STATE_Completed;
{
    ...
    if !(_state in [STATE_Completed, STATE_Failed]) then
    {
        ...
        if (_state isEqualTo STATE_Initialised) then
        {
            (((_this # 0) getVariable [_x, []])) params ["", "", "", "_canComplete", "_completed", "_completedServer"];
